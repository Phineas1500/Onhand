...
- **Constitution P8 vs. the marker-retry's visible revision** — **DECIDED 2026-08-02: buffer until final.** During any finalize retry (marker, PDF-anchor, missing-tool, learning-evidence), show only a working indicator; render the answer once, when settled. No draft text, no visible rewrite. Implementation pending (0.5.0 arc).
- **Notes: role-on-the-mark vs. role-in-chat** — **DECIDED 2026-08-02: the marks carry the depth.** Per G3, interpretive highlights get their 1–2 sentence notes (the global one-note cap is retired); chat shrinks to verdict + synthesis. "Place any longer detail in chat" is repealed. Implementation pending (0.5.0 arc).
