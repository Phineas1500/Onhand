...
| G14 | **Weak / unanswerable page** | **Say so honestly**, and **auto-open a better source by default — unless it's clearly not appropriate.** Research/comprehensiveness requests, and "nothing on-page or in open tabs answers," are the strongest triggers. **Never fabricate grounding** to look helpful. *(Implemented 2026-07-30: the runtime auto-opens/searches a background source when no open source supports a needed claim; an interim offer-first variant from 07-29 was replaced after the owner reaffirmed auto-open. When fetching is clearly inappropriate or fails, the claim is labeled general knowledge — see G7.)* *(Known tier difference, accepted 2026-07-31: cheap models — Luna, deepseek-v4-flash — do not self-initiate the fetch or the general-knowledge label, though they retrieve correctly when asked explicitly; a universal prompt rewrite (front-loaded decision procedure, same rule for all models) was tested on the honesty probe 3x and did not move Luna while Sol stayed correct. Per the accommodation lesson: free tier gets smaller budgets, never different rules — no per-model prompt fork. Regression probe: only OpenStax 16.8 open, ask what modern engineering says caused the Tacoma collapse; Sol searches and anchors on an opened source, cheap models answer from memory.)* | `[SETTLED]` |
