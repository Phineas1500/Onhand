...
- **Constitution P8 vs. the marker-retry's visible revision** — **DECIDED 2026-08-02: buffer until final.** During any finalize retry (marker, PDF-anchor, missing-tool, learning-evidence), show only a working indicator; render the answer once, when settled. No draft text, no visible rewrite. Landed 2026-08-02. *(Scoped 2026-08-09: gate-eligible turns — unambiguous marker prompts, plus a completed PDF read arming the anchor gate mid-turn — buffer the draft for the whole turn so the answer truly renders once; conversational turns keep streaming. Exposure on the unpredictable retries (missing-tool, transient-provider, blank-reply) is recorded via provisionalAnswerExposed rather than suppressed.)*
...
*v3.4 — 2026-08-09 (late night): prompt consolidation landed (v2.9 decision 7). Mark-policy wording that multiple prompt surfaces state now lives in one fragment module (src/agent/mark-policy.ts): the teach budget, note shape and job, the per-interpretive-mark note rule, notes-carry-the-depth, comparison per-side marks, the heading-marker ban, and the sidebar table policy. The constitution, launcher contract, both mark tool descriptions, the note schema, and the source-marker retry prompt compose these fragments; guard messages keep interpolating the enforced constants directly. The sweep itself caught live drift the 08-09 review missed: the constitution's multi-part bullet still carried the pre-v3.0 one-takeaway-note comparison rule (and one suite assertion was pinning that drifted wording as correct). A budget-sync tripwire now ties the fragment wording to the enforced constants, so a budget change must land in prose and guards together or fail the suite. Verified behavior-preserving: rendered-surface snapshot diff shows only the intended normalizations, all suites green, live teach (6/6 marks) and comparison (per-side notes, cap enforced) turns unchanged. Voice instructions intentionally not consolidated — that copy belongs to the voice-Learning redesign.*

*v3.2 — 2026-08-09 (evening): the live voice + Learning pass ran the graded-homework script on a practice-problems page and passed end to end — both mid-session toggle directions gate correctly at flip time, the never-reveal holds under a direct push, the §5.3 own-work carve-out answers directly, and ask-before-telling leads with a guiding question that reuses the existing anchor (G18 in voice). The pass surfaced and fixed two voice-path bugs: a response.create rejected mid-race replayed bare (losing the turn's grounded instructions and tool choice; the turn vanished from the sidebar), and Learning-mode Socratic turns ("quiz me", "don't tell me the answer") were trapped between the publish completeness gate (under-70-chars reads as a lead-in) and the never-reveal gate — after two forced complete-answer retries an empty publish fell through, narrated "no content to read aloud", and dropped the turn. Fixes: rejected responses replay with their options; explicit Socratic requests in Learning mode accept short question-form publishes; an empty publish after the retry budget rejects back to the model; voice failures log [onhand-voice] diagnostics; regressions pin the routing split (only Learning Socratic requests take the realtime path — everything else page-material routes through the typed backend and never meets these gates). Known stopgap gaps left for the voice-Learning redesign: realtime quiz turns cannot record checks (no learner-state continuity when the user answers the quiz), and the realtime path lacks the heading-highlight guard. Still owed: the Tacoma + Learning session review (the marks-carry-the-depth reading judgment).*

*v3.1 — 2026-08-09: post-review completion sweep of the v3.0 landing. An xhigh code review found the repeal incomplete and this sweep closed it: the retired 1–3-highlight/one-note budget was still live in the launcher contract, both mark tool descriptions, the source-marker retry prompt, and four guard messages (all now carry the 3–6-with-per-mark-notes policy); the comparison note guard capped at one note (now one per side) and a single failed note forfeited the whole budget (now two failures with none landed); the compact-teaching output ceiling rose 800 → 1600 so a batched 6/6 mark pass cannot truncate. Learning checks: meta-followups ("didn't I already answer that?") get an acknowledge-and-resolve directive instead of the false grading directive, and unanchored fallback checks are told to place a fresh highlight rather than scroll to a nonexistent annotationId. Voice: the mid-session Learning toggle re-send reads the live toggle, so the gates arrive/leave at flip time (pinned end-to-end). P8 follow-through: provisionalAnswerExposed is recorded again whenever a retry blanks streamed text, the transient-provider retry no longer leaks a stale prefix into the final reply, and buffering is scoped to gate-eligible turns — marker prompts buffer from the start, a completed PDF read arms buffering mid-turn, conversational turns keep streaming (see §9 P8 scope note). Validation: browser-runtime, sidebar, page-toolkit, and agent-runtime suites green.*

*v3.0 — 2026-08-02 (later): the v2.9 behavior arc landed. Marks carry the depth (per-interpretive-mark notes, teaching budget 3/1/460 → 6/6/800 (ceiling later 1600 — see v3.1), the tables ban narrowed to non-comparisons, "longer detail in chat" repealed across constitution/contract/schema); learning checks are graded on the page (the answer-detection now injects a grading directive — re-read the anchor, judge honestly, coach without revealing, record check_resolved — and an ungraded answer leaves the check open, pinned by regression); voice carries the Learning Mode gates when the toggle is on (ask-before-telling, the homework never-reveal with the §5.3 explanation and escape, don't-tell-me honored), re-sent on toggle change; all four finalize retries buffer until final (drafts clear to the working indicator; the reply-revising treatment is deleted). Validation: full suites green; live Tacoma + Learning session review still owed before the next release.*

*v2.9 — 2026-08-02: ethos audit round two (constitution, Learning Mode, UX surfaces) produced seven owner decisions in one sitting. (1) Depth lives on the page: notes per G3, one-note cap retired, chat shrinks — the §9 notes-role question is decided for marks. (2) Learning checks get graded on the page: answering a check runs a real turn that evaluates against the anchored passage; the regex auto-"correct" praise path is retired (it violated §6.9). (3) Voice gets the Learning Mode rules ported into its instructions as a stopgap until the voice-Learning redesign. (4) Teaching budgets rebalance to G2's advisory ~5–6 marks with notes per G3 and a higher token ceiling; small tables allowed for genuine comparisons. (5) Retry presentation buffers until final — the §9 P8 question is decided against visible revision. (6) Cold-start empty-state copy will promise mark-led answering. (7) Prompt consolidation is approved, sequenced after these changes land. Also queued as bugs: hardcoded demo-fixture answers in shipping prompts (multi-head attention canned prose/bonuses — delete), the learning evidence-net disarm, learner-state reset on toggle-off (§5.4), the missing §5.3 misclassification guard, provider-error JSON unwrapping, and silent citation-chip failures. None of this lands in 0.4.5 (mid-review); it is the 0.5.0 arc.*
