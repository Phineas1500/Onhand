...
| G12 | **Scope: use other tabs** | **Auto-use clearly-related open tabs** without asking first. *(Intentional evolution: more autonomous than the old offer-first behavior. Implemented in the runtime 2026-07-28: cross-tab retrieval is standard in every mode, tab tools are ungated, and the workspace scan carries tabIds for direct reads.)* | `[SETTLED]` |
