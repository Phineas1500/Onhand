...
- Start browser-only runtime checks with `npm run smoke:browser-runtime`.
...
- Use the extension side panel for authoritative browser-only UX checks.
...
These rules still apply to tests that explicitly exercise the old desktop app or localhost bridge. They should not be required for the browser-only side-panel prompt path.

- Prefer `Chrome Test` for authoritative GUI checks when running legacy browser-client flows.
- Prefer `sidebar submit` for end-to-end UX validation when a legacy runtime is involved.
- Use `desktop/API submit` only when the goal is a quick legacy runtime check.
- Prefer the managed local runtime for legacy desktop/bridge tests:
  - `npm run tmux:start`
  - `npm run tmux:status`
  - `npm run tmux:attach`
  - `npm run tmux:stop`
- Run `npm run test:preflight` before legacy desktop/bridge smokes.
...
Use this as the default browser-only smoke test. It does not start Electron or the localhost bridge.
...
- at least one browser tool call is executed
- the final side-panel state is `Reply ready`
- the latest turn contains the expected reply and page action
...
- minimum: this smoke after browser-runtime, provider, or side-panel routing changes
- add the manual side-panel smoke when UI wiring or Chrome extension reload behavior changed
