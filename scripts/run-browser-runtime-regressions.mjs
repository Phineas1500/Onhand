...
async function assertProviderApiKeyStorageAndRouting() {
	...
	assert.equal(getProviderModelOptions("onhand-free")[0].id, "gpt-6-luna", "free tier should pin its model");
...
async function main() {
	...
	await runSessionPageActionRestoreRegressions({ createOnhandBrowserRuntime: voiceRuntime.createOnhandBrowserRuntime });
