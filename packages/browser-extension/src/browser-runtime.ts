...
function selectToolsForPrompt(
	...
	if (wantsAllPorts) {
		...
		if (
			...
			textHasAny(text, /\b(compare|comparison|contrast|versus|vs\.?|differ|difference|agree|disagree)\b|\bother (?:paper|article|document|source|page)\b|\bboth (?:papers|articles|documents|sources|pages)\b/)
		) {
			add(TAB_TOOL_NAMES);
		}
		if (
...
function formatPdfCitationForModel(details: any) {
	...
	const lines = [
		...
		identifiers.suggestedUrl
			? `To open the cited work, navigate to ${identifiers.suggestedUrl} in a new tab (newTab: true) so the current paper stays open, then hand the PDF to the Onhand viewer.`
			: "The entry has no direct link; tell the user it could not be opened automatically.",
