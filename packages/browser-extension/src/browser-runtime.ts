...
async function hydrateLearningResearchPlanWithCorpus(plan: LearningResearchPlan | null, browserContextDetails: any, host: RuntimeHost) {
...
function selectToolsForPrompt(
	...
	if (wantsAllPorts) {
		...
		if (learningMode) {
			...
			// Learning Mode treats the browser as a workspace. Always make the
			// complete tab census and safe link-navigation tools available; prompt
			// wording should affect ranking, not whether cross-tab retrieval exists.
			add(TAB_TOOL_NAMES);
